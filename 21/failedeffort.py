...
def numericToDirectionalKeypad(numericcode, start = 'A'):
    ...
    for c in numericcode:
        ...
        if  pos[0] != 3:
            # moving moving right, move x then y.
        ...
        pos = dest
        start = c
    return start, [ d for d in "".join(decoded) ]
...
def DirToDirKeypadUp(start, dircode, map1 = True):
    #print(f"  DirToDirKeypadUp({start}, {dircode}, {map1})")
    dirdirpad = {
...
# +---+---+---+
    ...
    for c in dircode:
        ...
        pos = dest
    return pos, [ d for d in "".join(decoded) ]

def decode(combo):
    l0up = combo
    p0pos = 'A'
    p0pos, l1up = numericToDirectionalKeypad(l0up, p0pos)
    l1up = "".join(l1up)
    #l1up = "".join(numericToDirectionalKeypad(l0up, 'A'))
    p1pos, l2up = "".join(DirToDirKeypadUp('A', l1up))
    p2pos, l3   = "".join(DirToDirKeypadUp('A', l2up, False))
    print(f" l0: {combo} ")
...
['v>>A','>v>A','>>vA'],
['v>A','>vA'],
['v>v>A','v>>vA','>vv>A','>v>vA','>>vvA'],
['vv<<A','v<v<A','v<<vA','<vv<A','<v<vA','<<vvA'],
['vv<A','v<vA','<vvA'],
['vv>>A','v>v>A','v>>vA','>vv>A','>v>vA','>>vvA'],
['vv>A','v>vA','>vvA'],
['vv>v>A','vv>>vA','v>vv>A','v>v>vA','v>>vvA','>vvv>A','>vv>vA','>v>vvA','>>vvvA'],
['vv>vA','v>vvA','>vvvA'],
['vvv<A','vv<vA','v<vvA','<vvvA'],
['vvv>A','vv>vA','v>vvA','>vvvA']
]

for j, test in enumerate(testsubjects):
    transx = []
    print(f"\n===========\nTest: {j}, {len(test)} ")
    for i, code in enumerate(test):
        start = 'A'
        pos, lxn1 = DirToDirKeypadUp(start, code, True)
        start = 'A'
        pos, lxn2 = DirToDirKeypadUp(start, lxn1, True)
        print(f"  {len(lxn2)}, ", end="")
        transx.append((code, True, len(lxn2), "".join(lxn2)))
        # start = 'A'
        # pos, lxn2 = DirToDirKeypadUp(start, code, False)
        # print(f"  {len(lxn2)} ", end="")
        # transx.append((code, False, len(lxn2), lxn2))
    print()
    transx.sort(key = lambda x: x[2])
    i = 0
    print(f"\n  sorted?...")
    for x in transx:
        indent = ""
