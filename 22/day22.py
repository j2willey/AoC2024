...
def doSecretChallenge(filename = 'input.txt'):
    secretseeds = loadInitialSecrets(filename)
    ...
    for seed in secretseeds:
        ...
        for k, v in seqPrice.items():
            highBananas[k] += v
    return total, highBananas

def day22Part1(filename = 'input.txt'):
    total, highBananas = doSecretChallenge(filename)
    return total, "Part 1: total"

def day22Part2(filename = 'input.txt'):
    total, highBananas = doSecretChallenge(filename)
    maxBananas = max(highBananas.values())
    return maxBananas, "Part 2: max bananas"
