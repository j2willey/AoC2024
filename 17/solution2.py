...
def bootstrap(filename):
    ...
    print(f'Register A: {registerA}   B: {registerB}   C: {registerC}')
...
def dump_cpu():
    global pc, registerA, registerB, registerC, output
    print(f"   CPU State:   registerA: {registerA}  B: {registerB}  C: {registerC}")
    print(f"                program: {program}")
    print(f"                pc = {pc}    output: {output}")
...
def run_program(program):
    ...
    while pc < len(program):
        #print(f'pc: {pc}  program[pc]: {program[pc]} function: {opc2fxn[program[pc]]}')
...
def find_initial_registerA(rA, rB, start = 0):
    ...
    log = []
    print(f'  find_initial_registerA(rA: {rA} rB: {rB})')
    for r3A in range(start, 2048):
        ...
        reset_cpu()
        registerA = try_regA
        #print(f'   checking if registerA: {registerA}  => rB: {rB}')
        breakpoint = True
        run_program(program)
        breakpoint = False
    ...
    print(f'  No initial registerA found\n{"".join(log)}')
    return None


print("\n--------\n# Part 1:  test.txt")
bootstrap('test.txt')
print(" expect > 4,6,3,5,6,3,5,2,1,0 ")
run_program(program)
print(" output >", ','.join(output))

#print(f"\nregA: {registerA}  regB: {registerB}  regC: {registerC}")
print(f"\n--------\n## Part 1:  test.txt + regA = 2024")
reset_cpu()
registerA = 2024
print(f" expect > 4,2,5,6,7,7,7,7,3,1,0 ")
run_program(program)
print(" output >", ','.join(output))

print("\n--------\n# Part 1:  input.txt")
bootstrap('input.txt')
print(" expect=> 7,1,5,2,4,0,7,6,1 ")
run_program(program)
print(" output >", ','.join(output))

print(f"\n--------\n## Part 2:  test2.txt")
bootstrap('test2.txt')
print(f" expect=> {",".join([str(i) for i in program])} ")
run_program(program)
print(" output >", ','.join(output))
...
for i, rB in enumerate(program_reversed):
    ...
    while r3A < 2048:
        reset_cpu()
        print(f"expected state: regA: {regA}  rB: {rB}")
        ...
        run_program(program)
        print(" output >", ','.join(output), end="")
        if expect == output:
            print(f"   Success!!!")
            break
        else:
            print(f"   Failed:"  )
            print(f"   regA: {regA}  rB: {rB}  registerB: {registerB}  output: {output}")
            regA = regAbase
            r3A += 1
    print(f' regA: {regA}  rB: {rB}  r3A: {r3A}  output: {output}')
    print("    | loop |")
